...
- 单个依赖升级与卸载完成后改为显示 5 秒自动关闭的轻量提示，不再占用列表空间展示完整命令输出。
- 修复执行卸载时升级按钮错误进入加载状态的问题，升级、卸载与安装现在分别展示对应的执行状态。
- 新增右上角包管理任务中心，统一展示批量升级及单个升级、删除任务的等待、执行、成功和失败进度。
- 任务中心支持悬停临时展开、点击锁定展开；失败任务可单独重试，批量执行遇到失败时会继续处理后续任务。
